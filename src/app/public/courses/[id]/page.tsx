...
import { 
    ...
    ClipboardCheck, Award, MessageSquare, MessageCircle, User as UserIcon, Star, ShieldCheck, Clock, type LucideIcon
} from 'lucide-react';
import type { Course, Module, Lesson, WhatsIncludedItem } from '@/components/shared/CourseCard';
import { getCoursesAction } from '@/app/admin/courses/actions'; 
import { notFound } from 'next/navigation';
import type { ElementType } from 'react';

// Map icon names (strings) from data to actual Lucide components for rendering
const iconMap: { [key: string]: ElementType } = {
  Users, Award, Video, FileText, Brain, ClipboardCheck, Target, MessageSquare, MessageCircle, Clock, Check
};
...
export default async function CourseDetailPage({ params }: { params: { id: string } }) {
  ...
  const whatsIncluded = course.whatsIncluded?.map(item => ({
      ...item,
      icon: iconMap[item.icon as string] || Users // Default to Users icon if not found
  })) || [];
